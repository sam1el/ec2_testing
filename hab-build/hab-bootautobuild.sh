...
# Bump the package
cd ${buildlocation}
# curl -L -s -k https://github.com/stedolan/jq/releases/download/jq-1.6/jq-linux64 -o jq
# chmod +x jq
# hab pkg exec core/git git checkout ${gitcommit}
# echo "Checking published latest version of package at ${bldrurl}/v1/depot/pkgs/${origin}/audit_base/latest "
# pkgversion=$(curl ${bldrurl}/v1/depot/pkgs/${origin}/audit_base/latest | ./jq -j -r .ident.version)
# pkgversion_major=$(echo $pkgversion | cut -d'.' -f1)
# pkgversion_minor=$(echo $pkgversion | cut -d'.' -f2)
# pkgversion_patch=$(echo $pkgversion | cut -d'.' -f3)
# pkgversion_patchbump=$(expr $pkgversion_patch + 1)
# pkgversion_bumpfinal="${pkgversion_major}.${pkgversion_minor}.${pkgversion_patchbump}"
echo "Will build with pkgversion ${pkgversion}"
